...
problem appears in the path — open → fill → submit → wait → confirmation →
recovery. Walk each flow end to end in the order the user walks it, then judge
the screens.
...
a living action plan, not an archive. If you find an existing file, overwrite
it; what survived a closed audit becomes a pattern in the design standards and
an item in the project roadmap, and that's where it's looked up later.
...
- **Problem:** The dialog that confirms a purchase shows "Are you sure?" and
  ...
  knowing they're completing a transaction. The project's copy rule requires
  the verb on the confirm button.
- **Solution:**
  1. In `src/screens/ConfirmOrder.tsx`, in the `<ConfirmDialog>` at line 42,
     change `message` from `"Are you sure?"` to
...
- **Solution in numbered imperative steps**, with the exact string or snippet
  ...
  anything — not a variable name, not a class, not where to insert. If the
  change is too structural for this format, mark `**Executor:** architect`
  and explain why.
- **`Do not:` in every finding.** This is what stops the executor from
  expanding scope.
...
- **Batches are file-disjoint.** Two batches may not list the same file —
  that's what allows parallel execution. If two themes touch the same file,
  merge them into one batch. Aim for 3–6 findings per batch.
- **Order by severity** within a batch; order batches by maximum severity.
- Close with **`## Out of scope`**: what you saw, considered, and chose not
  to report (with the reason, one line each). This is as valuable as the
  findings — it prevents the next audit from reopening the same discussion.

## What not to do

- Do not edit code. Not even a label. Your product is the document.
- Do not invent a finding to make the document look complete. Nine real
  findings are worth more than forty, and the cost of each false one is a
  full agent executing it incorrectly.
- Do not propose a flow redesign inside a finding. If the flow is wrong at
  the root, open **one** high-severity finding describing the problem, mark
  `**Executor:** architect`, and don't try to detail the steps.
- Do not rewrite copy just to "sound better" without pointing to a concrete
  ambiguity.
