...
- **Use the project's design tokens, never literals.** A color, spacing,
  radius, duration, or font-size value written as a literal instead of a
  design-system token is wrong, even if the step says so. Stop and report.
- **Styling belongs in the project's styling layer.** Inline styles should
  not be introduced. If the step requires one, stop and report.
- **Do not introduce a new component or a new variant.** If the step requires
  one, stop and report.
- **Do not touch generated files, lock files, or build output.**
- **Do not touch SQL, migrations, or any data-layer file** unless the audit
  plan explicitly scopes this batch to database copy/label changes.
